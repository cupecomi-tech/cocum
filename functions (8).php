...
add_action('admin_footer', function() {
    ...
    // No ejecutar obtener_cuentas() al abrir Listado de Clientes o
    // Clientes a Cobrar (gd_place). Esas pantallas ahora únicamente leen
    // la información mediante cocum_calcular_cobranza_actual_usuario().
    // Mantener temporalmente la ejecución en MisCuentas para validar que
    // el registro y la sincronización de pagos continúen funcionando.
    if ($screen && $screen->post_type === 'miscuentas') {
        obtener_cuentas();
    }
});  ////////////////////ADD
...
function cocum_recalcular_reporte_diario_usuario($user_id) {
    ...
    reporte_diario();
    ob_end_clean();
    // Asegurar que las dos métricas visibles se guarden en la fila real de hoy,
    // independientemente de las ramas antiguas internas de reporte_diario().
    cocum_actualizar_metricas_cobranza_hoy($user_id);
    wp_set_current_user($prev_user_id);
}

// Función principal del cron: garantiza la fila de hoy y ejecuta el cálculo
// normal de reporte_diario() para cada cobrador, como si hubiera abierto la
// pantalla. El backfill histórico queda disponible, pero ya no se ejecuta
// automáticamente desde este cron.
function cocum_ejecutar_cron_backfill_reporte_diario() {
    ...
    // Marca de diagnóstico para comprobar la última ejecución real del evento.
    update_option('cocum_cron_reporte_diario_ultima_ejecucion', array(
        'fecha_hora'         => wp_date('Y-m-d H:i:s'),
        'usuarios_procesados' => count($authors),
    ), false);
}
 ...
 * podría alterar la fotografía de días anteriores.
 */
...
/**
 * Calcula las métricas actuales de "Clientes a cobrar" desde miscuentas.
 * Una persona de gd_place puede agrupar varias cuentas; por eso se cuenta cada
 * post de miscuentas pendiente y se suma la cuota de esa misma cuenta.
 *
 * Esta función es únicamente para el día presente. No debe utilizarse para
...
/**
 * Guarda las métricas centrales en la fila (o filas duplicadas) de hoy.
 * Solo toca "clientes_con_pagos" y "cobrar_dia_actual".
 */
function cocum_actualizar_metricas_cobranza_hoy($user_id) {
    $user_id = intval($user_id);
    if ($user_id <= 0) {
        return 0;
    }
    ...
    $hoy = wp_date('Y-m-d');
    $hoy_obj = DateTime::createFromFormat('Y-m-d', $hoy, wp_timezone());
    if (!$hoy_obj) {
        return 0;
    }
    ...
    $reportes_hoy = get_posts(array(
        ...
        'date_query'     => array(
            array(
                'year'  => intval($hoy_obj->format('Y')),
                'month' => intval($hoy_obj->format('m')),
                'day'   => intval($hoy_obj->format('d')),
            ),
        ),
    ));

    $actualizados = 0;
    foreach ($reportes_hoy as $reporte_id) {
        $reporte_id = intval($reporte_id);
        update_field('clientes_con_pagos', intval($metricas['cantidad']), $reporte_id);
        update_field('cobrar_dia_actual', floatval($metricas['total_cuotas']), $reporte_id);
        update_post_meta($reporte_id, '_cocum_reporte_generado', '1');
        update_post_meta($reporte_id, '_cocum_reporte_actualizado', wp_date('Y-m-d H:i:s'));
        $actualizados++;
    }
...
// Al abrir o recargar Reporte Diario, reflejar inmediatamente las métricas de hoy.
add_action('load-edit.php', function() {
    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if ($screen && $screen->post_type === 'reporte_diario') {
        cocum_actualizar_metricas_cobranza_hoy(get_current_user_id());
    }
}, 999);
...
function reporte_diario(){
   ...
   // $args_miscuentas2 = array( 'post_type' =>'miscuentas' , 'author' =>    $id_actual_ );
        ...
        foreach( $recent_postss2 as $recentt2){
              ...
              if($last_modified_date >= $last_post_date){
               ...
               // Usar la misma regla de la pantalla "Clientes a Cobrar": cuenta
               // activa y fecha próxima vencida/hoy, o estatus de pago vacío.
               $fecha_prox_timestamp_mc = strtotime(str_replace('/', '-', (string) $fecha_prox_cobro));
               $hoy_timestamp_mc = strtotime(str_replace('/', '-', (string) $hoyReporte));
               $es_pendiente_hoy_mc = (
                   $estatus_cuenta === "activa" &&
                   (
                       ($fecha_prox_timestamp_mc !== false && $hoy_timestamp_mc !== false && $fecha_prox_timestamp_mc <= $hoy_timestamp_mc) ||
                       trim((string) $estatus_pago) === ""
                   )
               );
               if($es_pendiente_hoy_mc){  //02/2024
    ...
    foreach( $recent_postss2 as $recentt2){
               ...
               // Usar exactamente la misma regla de la lista "Clientes a Cobrar".
               $fecha_prox_timestamp_mc = strtotime(str_replace('/', '-', (string) $fecha_prox_cobro));
               $hoy_timestamp_mc = strtotime(str_replace('/', '-', (string) $hoyReporte));
               $es_pendiente_hoy_mc = (
                   $estatus_cuenta === "activa" &&
               ...
               if($es_pendiente_hoy_mc){  //02/2024 se agrego fecha prox para que solo aparesca los que se cobraran hoy
 ...
 if ($post_reporteDiario_pen && count($post_reporteDiario_pen) >= 2) {
  ...
  ///////////Actualizacion o insercion de fila en tabla Reporte Diario////////////////////////
    ...
    // Fuente única para las dos métricas actuales: cantidad de cuentas y suma
    // de las cuotas de esas mismas cuentas. Sustituye los recorridos antiguos
    // únicamente al guardar el reporte de hoy.
    $cobranza_actual = cocum_calcular_cobranza_actual_usuario($id_actual_);
    $clientes_por_cobrar = intval($cobranza_actual['cantidad']);
    $sumasTotalesPagos = floatval($cobranza_actual['total_cuotas']);
    ...
    // La fila de hoy ya fue calculada por el flujo normal y puede considerarse
    // una fotografía válida cuando pase a ser histórica.
    update_post_meta($id_reporteDiario, '_cocum_reporte_generado', '1');
    update_post_meta($id_reporteDiario, '_cocum_reporte_actualizado', wp_date('Y-m-d H:i:s'));
...
if(is_null($id_nl)) {
    ...
    // Nueva Liquidada ya no reconstruye automáticamente filas históricas de
    // Reporte Diario. Por ahora trabaja normalmente con las filas existentes.
    // La detección, el aviso y la autorización manual se agregarán después.
...
function crear_cuenta_cliente($post_id) {
    ...
    // Evitar ejecuciones internas que no representan una cuenta nueva real.
    ...
    if (get_post_type($post_id) !== 'cliente' || get_post_status($post_id) !== 'publish') {
        return;
    }

    // Una persona puede tener varias cuentas, pero cada post "cliente" debe
    // generar una sola fila de "miscuentas". La comparación se hace por el
    // ID individual de la cuenta, nunca por el nombre ni por el cliente padre.
    $cuenta_existente = get_posts(array(
        'post_type'      => 'miscuentas',
        'post_status'    => 'any',
        'posts_per_page' => 1,
        'fields'         => 'ids',
...
function agrupar_nombres_repetidos_en_admin() {
    ...
    jQuery(document).ready(function($) {
        ...
        rows.each(function(index) {
            ...
            // nombre debe abrir directamente su historial en miscuentas.
            if (rowMode === 'terminated' && $link.length) {
                try {
                    const historyUrl = new URL($link.attr('href'), window.location.origin);
                    historyUrl.searchParams.set('ver_historial', '1');
                    $link.attr('href', historyUrl.toString());
                } catch (error) {
                    const separator = String($link.attr('href') || '').indexOf('?') === -1 ? '?' : '&';
                    $link.attr('href', String($link.attr('href') || '') + separator + 'ver_historial=1');
                }
            }
...
add_action( 'admin_footer', function() {
    ...
    echo '<a id="' . esc_attr($btn_id) . '" class="page-title-action" href="' . esc_url($url) . '" style="color: ' . esc_attr($btn_color) . ' !important; font-weight: bold !important; border: 2px solid ' . esc_attr($btn_color) . ' !important; padding: 4px 12px !important; border-radius: 4px !important; display: inline-block !important; text-decoration: none !important; font-size: 12px !important; margin-left: 12px !important; vertical-align: middle !important; line-height: 1.4 !important; white-space: nowrap !important;">
        ' . esc_html($label) . '
    </a>';
    ?>
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            function acomodarBotonHistorial() {
                ...
                // Ubicación principal: junto al título superior de Miscuentas.
                // Siempre existe, incluso cuando una búsqueda no devuelve filas.
                var tituloPagina = $('body.post-type-miscuentas .wrap h1.wp-heading-inline, body.post-type-miscuentas .wrap h1').first();
                if (tituloPagina.length) {
                    miBoton.insertAfter(tituloPagina);
                    return;
                }
                
                // Respaldo 1: colocarlo junto al toggle de "Inline Edit".
                ...
                // Respaldo 2: buscar el botón "Reset Sorting".
                ...
                // Respaldo 3: usar la barra de herramientas principal.
...
/**
 * Envía los roles administrativos personalizados a la lista de usuarios
 * cuando intentan abrir el Escritorio principal de WordPress.
 */
function cocum_enviar_roles_a_usuarios() {
    ...
    $roles_redirigidos = array( 'admin', 'super_admin' );

    if ( !array_intersect( $roles_redirigidos, (array) $usuario->roles ) ) {
        return;
    }
...
add_action( 'load-index.php', 'cocum_enviar_roles_a_usuarios' );
...
function cocum_guardar_fecha_registro($post_id) {
    ...
    // wp_update_post() vuelve a disparar save_post. Desconectamos solamente
    // durante esta actualización interna al creador de miscuentas para que el
    // mismo crédito no se inserte dos veces.
    $prioridad_crear_cuenta = has_action('save_post', 'crear_cuenta_cliente');
    if (false !== $prioridad_crear_cuenta) {
        remove_action('save_post', 'crear_cuenta_cliente', $prioridad_crear_cuenta);
    }
    ...
    if (false !== $prioridad_crear_cuenta) {
        add_action('save_post', 'crear_cuenta_cliente', $prioridad_crear_cuenta, 2);
    }
...
// No reconstruir automáticamente una fotografía histórica que ya contiene
// información. El error anterior consideraba como hueco un día válido cuando
// había importe por cobrar pero nadie había pagado.
if ($fecha_ymd < wp_date('Y-m-d')) {
    $ya_generado = (string) get_post_meta($rd_id, '_cocum_reporte_generado', true);
    $campos_snapshot = array(
        'total_cobrado',
        'cobrar_dia_actual',
        'total_prestado',
        'clientes_con_pagos',
    ...
    $tiene_datos = false;
    foreach ($campos_snapshot as $campo_snapshot) {
        if (abs(floatval(get_field($campo_snapshot, $rd_id))) > 0) {
            $tiene_datos = true;
            break;
        }
    }

    if ($ya_generado === '1' || $tiene_datos) {
        return true;
    }
}
...
update_post_meta($rd_id, '_cocum_reporte_generado', '1');
update_post_meta($rd_id, '_cocum_reporte_actualizado', wp_date('Y-m-d H:i:s'));
...
function cocum_es_hueco_reporte_diario_post($rd_id) {
    ...
    if ((string) get_post_meta($rd_id, '_cocum_reporte_generado', true) === '1') {
        return false;
    }
    ...
    // Solo es hueco cuando todos los valores relevantes están en cero. Tener
    // cero cobrado puede ser un resultado válido: significa que nadie pagó.
    $es_hueco = (
        $cobrar <= 0.0 &&
        $clientes <= 0 &&
        $total_cobrado <= 0.0 &&
...
function cocum_recalcular_huecos_reporte_diario($user_id, $fecha_inicio_ymd = null, $fecha_fin_ymd = null, $limite = 10) {
    ...
    foreach ($reportes as $rd_id) {
        ...
        $rd_id = intval($rd_id);
        // Fuente única para decidir si una fila necesita reconstrucción. Una
        // fila marcada como generada nunca es hueco, incluso cuando todos sus
        // movimientos legítimos del día hayan sido cero.
        if (!cocum_es_hueco_reporte_diario_post($rd_id)) {
            continue;
        }
...
add_action('load-edit.php', function() {
    $screen = get_current_screen();
    if ($screen && $screen->post_type === 'reporte_diario') {
        // La pantalla solo actualiza el reporte normal de hoy. Las fechas
        // históricas se reconstruyen exclusivamente desde el botón manual.
        reporte_diario();
    }
});
...
/**
 * Agrupa por fecha los pagos de MisCuentas del usuario conectado.
 * Cada cuenta se consulta una sola vez para todo el rango solicitado.
 */
function cocum_totales_miscuentas_por_fecha_usuario($user_id, $inicio, $fin) {
    $totales = array();
    $miscuentas_ids = get_posts(array(
        'post_type'      => 'miscuentas',
    ...
    foreach ($miscuentas_ids as $miscuenta_id) {
        $resumen = cocum_calcular_pagos_miscuentas_por_rango(intval($miscuenta_id), $inicio, $fin);
        foreach ($resumen['detalles_pagos'] as $pago) {
            $fecha_pago = isset($pago['fecha']) ? cocum_normalizar_fecha_ymd($pago['fecha']) : '';
            if ($fecha_pago === '' || $fecha_pago < $inicio || $fecha_pago > $fin) {
                continue;
            }
            if (!isset($totales[$fecha_pago])) {
                $totales[$fecha_pago] = 0.0;
            }
            $totales[$fecha_pago] += floatval($pago['monto']);
        }
    }
...
/**
 * Devuelve fotografías inexistentes o cuyo Total Cobrado no coincide con
 * MisCuentas. Solo consulta al usuario conectado y nunca corrige automáticamente.
 */
add_action('wp_ajax_cocum_buscar_reportes_fallidos', function () {
    check_ajax_referer('cocum_reportes_fallidos', 'nonce');

    if (!is_user_logged_in() || !current_user_can('read')) {
        wp_send_json_error(array('mensaje' => 'No tienes permiso para realizar esta consulta.'), 403);
    }

    $user_id = get_current_user_id();
    $inicio = isset($_POST['fecha_inicio']) ? sanitize_text_field(wp_unslash($_POST['fecha_inicio'])) : '';
    $fin = isset($_POST['fecha_fin']) ? sanitize_text_field(wp_unslash($_POST['fecha_fin'])) : '';

    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $inicio) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $fin)) {
        wp_send_json_error(array('mensaje' => 'Selecciona un rango de fechas válido.'), 400);
    }

    $tz = wp_timezone();
    $inicio_obj = DateTimeImmutable::createFromFormat('!Y-m-d', $inicio, $tz);
    $fin_obj = DateTimeImmutable::createFromFormat('!Y-m-d', $fin, $tz);
    $hoy_obj = new DateTimeImmutable('today', $tz);

    if (!$inicio_obj || !$fin_obj || $inicio_obj->format('Y-m-d') !== $inicio || $fin_obj->format('Y-m-d') !== $fin) {
        wp_send_json_error(array('mensaje' => 'El rango contiene una fecha inválida.'), 400);
    }
    if ($inicio_obj > $fin_obj) {
        wp_send_json_error(array('mensaje' => 'La fecha inicial no puede ser posterior a la fecha final.'), 400);
    }

    // El día actual todavía está en proceso y no puede considerarse fallido.
    $ayer_obj = $hoy_obj->modify('-1 day');
    if ($fin_obj > $ayer_obj) {
        $fin_obj = $ayer_obj;
        $fin = $fin_obj->format('Y-m-d');
    }
    if ($inicio_obj > $fin_obj) {
        wp_send_json_success(array('faltantes' => array(), 'mensaje' => 'No hay días históricos dentro del rango seleccionado.'));
    }

    $dias_rango = intval($inicio_obj->diff($fin_obj)->days);
    if ($dias_rango > 366) {
        wp_send_json_error(array('mensaje' => 'Selecciona un rango máximo de 366 días.'), 400);
    }

    $reportes_ids = get_posts(array(
        'post_type'      => 'reporte_diario',
        'author'         => $user_id,
        'posts_per_page' => -1,
        'post_status'    => 'publish',
    ...
    $totales_fuente = cocum_totales_miscuentas_por_fecha_usuario($user_id, $inicio, $fin);
    $faltantes = array();
    for ($cursor = $inicio_obj; $cursor <= $fin_obj; $cursor = $cursor->modify('+1 day')) {
        $fecha_ymd = $cursor->format('Y-m-d');
        $total_fuente = isset($totales_fuente[$fecha_ymd]) ? round(floatval($totales_fuente[$fecha_ymd]), 2) : 0.0;
        if (!isset($reportes_por_fecha[$fecha_ymd])) {
            $faltantes[] = array(
                'fecha'         => $fecha_ymd,
                'fecha_mostrar' => wp_date('d/m/Y', $cursor->getTimestamp(), $tz),
                'tipo'          => 'faltante',
                'estado'        => 'No existe fotografía',
                'total_reporte' => null,
        ...
        $total_reporte = round(floatval($reportes_por_fecha[$fecha_ymd]['total_cobrado']), 2);
        $diferencia = round($total_fuente - $total_reporte, 2);
        if (abs($diferencia) >= 0.01) {
            $faltantes[] = array(
                'fecha'         => $fecha_ymd,
                'fecha_mostrar' => wp_date('d/m/Y', $cursor->getTimestamp(), $tz),
                'tipo'          => 'diferencia',
                'estado'        => 'Total cobrado no coincide',
                'reporte_id'    => intval($reportes_por_fecha[$fecha_ymd]['id']),
...
/**
 * Corrige únicamente los campos económicos dependientes del Total Cobrado de
 * una fotografía existente. No reconstruye otras fechas ni modifica préstamos,
 * gastos, clientes a cobrar, base o inyección.
 */
add_action('wp_ajax_cocum_corregir_reporte_cobrado', function () {
    check_ajax_referer('cocum_reportes_fallidos', 'nonce');

    if (!is_user_logged_in() || !current_user_can('read')) {
        wp_send_json_error(array('mensaje' => 'No tienes permiso para corregir este reporte.'), 403);
    }

    $user_id = get_current_user_id();
    $fecha = isset($_POST['fecha']) ? sanitize_text_field(wp_unslash($_POST['fecha'])) : '';
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fecha)) {
        wp_send_json_error(array('mensaje' => 'La fecha recibida no es válida.'), 400);
    }

    $tz = wp_timezone();
    $fecha_obj = DateTimeImmutable::createFromFormat('!Y-m-d', $fecha, $tz);
    $hoy_obj = new DateTimeImmutable('today', $tz);
    if (!$fecha_obj || $fecha_obj->format('Y-m-d') !== $fecha || $fecha_obj >= $hoy_obj) {
        wp_send_json_error(array('mensaje' => 'Solo se pueden corregir fechas históricas anteriores a hoy.'), 400);
    }
    ...
    $reportes = get_posts(array(
        ...
        'no_found_rows'  => true,
        'date_query'     => array(array(
            'year'  => intval($fecha_obj->format('Y')),
            'month' => intval($fecha_obj->format('m')),
            'day'   => intval($fecha_obj->format('d')),
        )),
    ));

    if (empty($reportes)) {
        wp_send_json_error(array('mensaje' => 'La fotografía no existe. Utiliza primero el botón Reconstruir.'), 404);
    }

    $reporte_id = intval($reportes[0]);
    $totales_fuente = cocum_totales_miscuentas_por_fecha_usuario($user_id, $fecha, $fecha);
    $total_nuevo = isset($totales_fuente[$fecha]) ? round(floatval($totales_fuente[$fecha]), 2) : 0.0;
    $total_anterior = round(floatval(get_field('total_cobrado', $reporte_id)), 2);

    if (abs($total_nuevo - $total_anterior) < 0.01) {
        wp_send_json_success(array('mensaje' => 'El Total Cobrado ya coincide con MisCuentas. No fue necesario modificarlo.'));
    }

    $diferencia = round($total_nuevo - $total_anterior, 2);
    $porcentaje = floatval(get_option('comision_cobrador_porcentaje', 3));
    $comision_nueva = round($total_nuevo * ($porcentaje / 100), 2);
    $caja_actual_anterior = floatval(get_field('caja_actual', $reporte_id));
    $caja_actual_nueva = round($caja_actual_anterior + $diferencia, 2);
    $total_prestado = floatval(get_field('total_prestado', $reporte_id));
    $base = floatval(get_field('base', $reporte_id));
    $caja_dia_nueva = round($total_nuevo - $total_prestado + $base, 2);

    update_field('total_cobrado', $total_nuevo, $reporte_id);
    update_field('comision', $comision_nueva, $reporte_id);
    update_field('caja_actual', $caja_actual_nueva, $reporte_id);
    update_post_meta($reporte_id, 'caja_dia', $caja_dia_nueva);

    update_post_meta($reporte_id, '_cocum_correccion_cobro_manual', wp_date('Y-m-d H:i:s'));
    update_post_meta($reporte_id, '_cocum_correccion_cobro_por', $user_id);
    update_post_meta($reporte_id, '_cocum_correccion_cobro_anterior', $total_anterior);
    update_post_meta($reporte_id, '_cocum_correccion_cobro_nuevo', $total_nuevo);

    wp_send_json_success(array(
        'mensaje'        => 'Reporte del ' . wp_date('d/m/Y', $fecha_obj->getTimestamp(), $tz) . ' corregido con MisCuentas.',
...
/**
 * Reconstruye una sola fecha faltante, únicamente para el usuario conectado.
 */
add_action('wp_ajax_cocum_reconstruir_reporte_fallido', function () {
    check_ajax_referer('cocum_reportes_fallidos', 'nonce');

    if (!is_user_logged_in() || !current_user_can('read')) {
        wp_send_json_error(array('mensaje' => 'No tienes permiso para reconstruir este reporte.'), 403);
    }
    ...
    $hoy_obj = new DateTimeImmutable('today', $tz);
    if (!$fecha_obj || $fecha_obj->format('Y-m-d') !== $fecha || $fecha_obj >= $hoy_obj) {
        wp_send_json_error(array('mensaje' => 'Solo se pueden reconstruir fechas históricas anteriores a hoy.'), 400);
    }
    ...
    if (!empty($existentes)) {
        wp_send_json_error(array('mensaje' => 'La fotografía de esa fecha ya existe y no fue modificada.'), 409);
    }

    $resultado = cocum_recalcular_reporte_diario_fecha($user_id, $fecha);
    if (!$resultado) {
        wp_send_json_error(array('mensaje' => 'No fue posible reconstruir el reporte. No se modificaron otras fechas.'), 500);
    }

    $creados = get_posts(array(
        'post_type'      => 'reporte_diario',
        'author'         => $user_id,
        'posts_per_page' => 1,
        'post_status'    => 'publish',
    ...
    if (empty($creados)) {
        wp_send_json_error(array('mensaje' => 'El proceso terminó, pero no se encontró la fotografía creada.'), 500);
    }

    update_post_meta(intval($creados[0]), '_cocum_reconstruccion_manual', wp_date('Y-m-d H:i:s'));
    update_post_meta(intval($creados[0]), '_cocum_reconstruido_por', $user_id);

    wp_send_json_success(array(
        'mensaje'    => 'Reporte del ' . wp_date('d/m/Y', $fecha_obj->getTimestamp(), $tz) . ' reconstruido correctamente.',
        'reporte_id' => intval($creados[0]),
    ));
});

/**
 * Interfaz de Reportes fallidos en la pantalla de Reporte Diario.
 */
add_action('admin_footer', function () {
    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if (!$screen || $screen->post_type !== 'reporte_diario') {
    ...
    $tz = wp_timezone();
    $ayer = new DateTimeImmutable('yesterday', $tz);
    $inicio = $ayer->modify('-29 days');
    $nonce = wp_create_nonce('cocum_reportes_fallidos');
    ?>
    <style>
        #cocum-fallidos-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:100000;padding:30px;box-sizing:border-box}
        #cocum-fallidos-overlay.cocum-abierto{display:flex;align-items:center;justify-content:center}
        #cocum-fallidos-modal{width:min(820px,100%);max-height:90vh;overflow:auto;background:#fff;border-radius:8px;box-shadow:0 12px 35px rgba(0,0,0,.3);padding:20px;box-sizing:border-box}
        .cocum-fallidos-cabecera{display:flex;align-items:center;justify-content:space-between;gap:15px}
        .cocum-fallidos-cabecera h2{margin:0}
        .cocum-fallidos-cerrar{border:0;background:transparent;font-size:25px;cursor:pointer;line-height:1}
        .cocum-fallidos-filtros{display:flex;align-items:end;gap:12px;flex-wrap:wrap;margin:20px 0}
        .cocum-fallidos-filtros label{display:flex;flex-direction:column;gap:5px;font-weight:600}
        .cocum-fallidos-tabla{width:100%;border-collapse:collapse}
        .cocum-fallidos-tabla th,.cocum-fallidos-tabla td{padding:9px;border:1px solid #dcdcde;text-align:left}
        .cocum-fallidos-tabla th{background:#f0f0f1}
        .cocum-fallidos-estado{padding:10px 12px;margin:12px 0;border-left:4px solid #dba617;background:#fff8e5}
        .cocum-fallidos-exito{border-left-color:#00a32a;background:#edfaef}
        .cocum-fallidos-error{border-left-color:#d63638;background:#fcf0f1}
        @media(max-width:600px){#cocum-fallidos-overlay{padding:10px}.cocum-fallidos-tabla{font-size:12px}}
    </style>
    ...
    <div id="cocum-fallidos-overlay" role="dialog" aria-modal="true" aria-labelledby="cocum-fallidos-titulo">
        <div id="cocum-fallidos-modal">
            ...
            <div class="cocum-fallidos-cabecera">
                ...
                <button type="button" class="cocum-fallidos-cerrar" aria-label="Cerrar">&times;</button>
            </div>
            <p>Busca fotografías inexistentes o cuyo Total Cobrado no coincide con los pagos guardados en MisCuentas. Nada se modifica sin tu autorización.</p>
            <div class="cocum-fallidos-filtros">
                <label>Fecha inicial
                    <input type="date" id="cocum-fallidos-inicio" value="<?php echo esc_attr($inicio->format('Y-m-d')); ?>" max="<?php echo esc_attr($ayer->format('Y-m-d')); ?>">
                </label>
                <label>Fecha final
                    <input type="date" id="cocum-fallidos-fin" value="<?php echo esc_attr($ayer->format('Y-m-d')); ?>" max="<?php echo esc_attr($ayer->format('Y-m-d')); ?>">
                </label>
                <button type="button" class="button button-primary" id="cocum-fallidos-buscar">Buscar reportes fallidos</button>
            </div>
            <div id="cocum-fallidos-mensaje" class="cocum-fallidos-estado" style="display:none"></div>
            <div id="cocum-fallidos-resultados"></div>
        </div>
    </div>

    <script>
    document.addEventListener('DOMContentLoaded', function () {
        const overlay = document.getElementById('cocum-fallidos-overlay');
        const resultados = document.getElementById('cocum-fallidos-resultados');
        const mensaje = document.getElementById('cocum-fallidos-mensaje');
        ...
        function mostrarMensaje(texto, tipo) {
            ...
            mensaje.className = 'cocum-fallidos-estado' + (tipo ? ' cocum-fallidos-' + tipo : '');
            mensaje.textContent = texto;
        }

        function instalarBoton() {
            if (document.getElementById('cocum-boton-reportes-fallidos')) return true;
            const referencia = document.getElementById('cocum-boton-ver-clientes') || document.getElementById('btn-open-comision');
            if (!referencia) return false;
            const boton = document.createElement('button');
            boton.type = 'button';
            boton.id = 'cocum-boton-reportes-fallidos';
            boton.className = 'page-title-action';
            boton.textContent = 'Reportes fallidos';
            boton.style.setProperty('display', 'inline-block', 'important');
            boton.style.setProperty('margin-left', '8px', 'important');
        ...
        if (!instalarBoton()) {
            ...
            const temporizador = setInterval(function () {
                intentos++;
                if (instalarBoton() || intentos >= 30) clearInterval(temporizador);
            }, 100);
        }

        function buscarFallidos() {
            const inicio = document.getElementById('cocum-fallidos-inicio').value;
            const fin = document.getElementById('cocum-fallidos-fin').value;
            resultados.innerHTML = '';
            mostrarMensaje('Comparando Reporte Diario con MisCuentas...', '');

            const datos = new FormData();
            datos.append('action', 'cocum_buscar_reportes_fallidos');
            datos.append('nonce', nonce);
            datos.append('fecha_inicio', inicio);
            datos.append('fecha_fin', fin);

            fetch(ajaxUrl, {method:'POST', credentials:'same-origin', body:datos})
                .then(respuesta => respuesta.json())
                .then(json => {
                    if (!json.success) throw new Error(json.data && json.data.mensaje ? json.data.mensaje : 'No fue posible consultar los reportes.');
                    const filas = json.data.faltantes || [];
                    if (!filas.length) {
                        mostrarMensaje(json.data.mensaje || 'No se encontraron reportes faltantes ni diferencias en este rango.', 'exito');
                        return;
                    }
                    mostrarMensaje('Se encontraron ' + filas.length + ' fecha(s) que requieren revisión.', '');
                    let html = '<table class="cocum-fallidos-tabla"><thead><tr><th>Fecha</th><th>Estado</th><th>Reporte Diario</th><th>MisCuentas</th><th>Diferencia</th><th>Acción</th></tr></thead><tbody>';
                    filas.forEach(fila => {
                        const dinero = valor => valor === null || typeof valor === 'undefined' ? '—' : '$' + Number(valor).toFixed(2);
                        let accion = '';
                        if (fila.tipo === 'diferencia') {
                            accion = '<button type="button" class="button button-primary cocum-corregir-cobrado" data-fecha="' + escapar(fila.fecha) + '" data-mostrar="' + escapar(fila.fecha_mostrar) + '" data-anterior="' + escapar(fila.total_reporte) + '" data-nuevo="' + escapar(fila.total_fuente) + '">Corregir valores</button>';
                        } else {
                            accion = '<button type="button" class="button cocum-reconstruir-fallido" data-fecha="' + escapar(fila.fecha) + '" data-mostrar="' + escapar(fila.fecha_mostrar) + '">Reconstruir</button>';
                        }
                        html += '<tr><td>' + escapar(fila.fecha_mostrar) + '</td><td>' + escapar(fila.estado) + '</td><td>' + dinero(fila.total_reporte) + '</td><td>' + dinero(fila.total_fuente) + '</td><td>' + dinero(fila.diferencia) + '</td><td>' + accion + '</td></tr>';
                    });
                    html += '</tbody></table>';
        ...
        document.getElementById('cocum-fallidos-buscar').addEventListener('click', buscarFallidos);

        resultados.addEventListener('click', function (event) {
            const botonCorregir = event.target.closest('.cocum-corregir-cobrado');
            if (botonCorregir) {
                const fecha = botonCorregir.dataset.fecha;
                const mostrar = botonCorregir.dataset.mostrar;
                const anterior = Number(botonCorregir.dataset.anterior).toFixed(2);
                const nuevo = Number(botonCorregir.dataset.nuevo).toFixed(2);
                const confirmacion = '¿Corregir el Reporte Diario del ' + mostrar + '?\n\nTotal cobrado actual: $' + anterior + '\nTotal según MisCuentas: $' + nuevo + '\n\nTambién se ajustarán Comisión, Caja actual y Caja del día de esa fotografía. No se modificarán préstamos, gastos, clientes ni otras fechas.';
                if (!window.confirm(confirmacion)) return;

                botonCorregir.disabled = true;
                botonCorregir.textContent = 'Corrigiendo...';
                const datosCorreccion = new FormData();
                datosCorreccion.append('action', 'cocum_corregir_reporte_cobrado');
                datosCorreccion.append('nonce', nonce);
                datosCorreccion.append('fecha', fecha);

                fetch(ajaxUrl, {method:'POST', credentials:'same-origin', body:datosCorreccion})
                    .then(respuesta => respuesta.json())
                    .then(json => {
                        if (!json.success) throw new Error(json.data && json.data.mensaje ? json.data.mensaje : 'No fue posible corregir el reporte.');
                        mostrarMensaje(json.data.mensaje, 'exito');
                        buscarFallidos();
                    })
                    .catch(error => {
                        mostrarMensaje(error.message, 'error');
                        botonCorregir.disabled = false;
                        botonCorregir.textContent = 'Corregir valores';
                    });
                return;
            }

            const boton = event.target.closest('.cocum-reconstruir-fallido');
            if (!boton) return;
            const fecha = boton.dataset.fecha;
            const mostrar = boton.dataset.mostrar;
            if (!window.confirm('¿Autorizar la reconstrucción del Reporte Diario del ' + mostrar + '? Solo se procesará esta fecha y tu usuario.')) return;

            boton.disabled = true;
            boton.textContent = 'Reconstruyendo...';
            const datos = new FormData();
            datos.append('action', 'cocum_reconstruir_reporte_fallido');
            datos.append('nonce', nonce);
            ...
            fetch(ajaxUrl, {method:'POST', credentials:'same-origin', body:datos})
                .then(respuesta => respuesta.json())
                .then(json => {
                    if (!json.success) throw new Error(json.data && json.data.mensaje ? json.data.mensaje : 'No fue posible reconstruir el reporte.');
                    mostrarMensaje(json.data.mensaje, 'exito');
                    buscarFallidos();
                })
                .catch(error => {
                    ...
                    boton.textContent = 'Reconstruir';
                });
        });

        overlay.addEventListener('click', function (event) {
            if (event.target === overlay || event.target.closest('.cocum-fallidos-cerrar')) {
                overlay.classList.remove('cocum-abierto');
                document.body.style.overflow = '';
            }
        });
